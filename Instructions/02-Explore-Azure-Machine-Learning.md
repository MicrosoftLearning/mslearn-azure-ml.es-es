...
1. Fíjese en la sección **Autor**, que incluye **Cuadernos**, **ML automatizado**, y **Diseñador**. Estas son las tres formas en que puede crear sus propios modelos de aprendizaje automático dentro de la Estudio de Azure Machine Learning.
...
1. Tenga en cuenta la sección **Administrar**, que incluye los **almacenes de datos** y **proceso**, entre otras cosas. Estos son recursos infraestructurales necesarios para entrenar o implementar un modelo de aprendizaje automático. 
...
> [!Note]
> Es posible que aparezcan ventanas emergentes que le guiarán por el estudio. Puede cerrar e ignorar todos los elementos emergentes y centrarse en las instrucciones de este laboratorio.
...
1. Seleccione **Enviar** en la parte superior de la página. Aparece un error ya que aún no ha seleccionado un destino de proceso. La canalización no se puede ejecutar sin recursos de proceso. 
...
1. En el Estudio de Azure Machine Learning, seleccione la página **Proceso** en el menú del lado izquierdo. Puede usar cuatro tipos de recursos de proceso:
    ...
    - **Clústeres de inferencia**: un clúster de Kubernetes usado durante la inferencia. Ideal para la implementación de modelos en tiempo real a gran escala.
...
1. Seleccione la pestaña **Precompilado clásico**.
1. Seleccione el borrador de canalización **Regresión - Predicción de precios de automóviles (básico)** .
1. Seleccione **Configuración** en la parte superior derecha para expandir el panel **Configuración**.
1. Seleccione **Instancia de proceso** en **Seleccionar tipo de cálculo**.
1. Seleccione la instancia de proceso recién creada en **Seleccionar clúster de proceso de Azure ML**. 
1. Seleccione **Enviar** para volver a ejecutar la canalización de entrenamiento.
1. Aparecerá un elemento emergente para configurar el trabajo de canalización. Configure y envíe un nuevo trabajo de canalización con las siguientes opciones:
    - **Experimentación**: `Create new`
    - **Nombre del experimento nuevo**: `train-regression-designer`
    - Mantenga el resto de configuración predeterminada.
